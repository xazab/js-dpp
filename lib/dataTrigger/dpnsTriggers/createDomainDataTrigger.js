...
async function createDomainDataTrigger(documentTransition, context, topLevelIdentity) {
  ...
  if (records.xazabUniqueIdentityId
    && !context.getOwnerId().equals(records.xazabUniqueIdentityId)
  ) {
    result.addError(
      new DataTriggerConditionError(
        documentTransition,
        context.getDataContract(),
        context.getOwnerId(),
        'ownerId doesn\'t match xazabUniqueIdentityId',
      ),
    );
  }

  if (records.xazabAliasIdentityId
    && !context.getOwnerId().equals(records.xazabAliasIdentityId)) {
    result.addError(
      new DataTriggerConditionError(
        documentTransition,
        context.getDataContract(),
        context.getOwnerId(),
        'ownerId doesn\'t match xazabAliasIdentityId',
      ),
    );
  }
